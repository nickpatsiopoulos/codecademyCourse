...
//routes are the API calls - each route is the end of the URL and the list of objects under each route are the CRUD functions that are possible
//Need to add comments routes
const routes = {
  '/users': {
...
//new - need to create createComment function
  '/comments': {
    'POST': createComment
  },
//new - need to create update and delete Comment functions
'/comments/:id': {
  'PUT': updateComment,
  'DELETE': deleteComment
  },
//new - need to create upvote Comment functions
'/comments/:id/upvote': {
    'PUT': upvoteComment
  },
  //new - need to create downvote Comment functions
  '/comments/:id/downvote': {
    'PUT': downvoteComment
  }
}

//checks if there is a user and returns all of their articles, comments, need to write code for comment IDs
function getUser(url, request) {
//splits the request URL to find the username
  const username = url.split('/').filter(segment => segment)[1];
//sets the user equal to what's in the database for that user
  const user = database.users[username];
  const response = {};

  if (user) {
//retrieve all article from identified user
    const userArticles = user.articleIds.map(
        articleId => database.articles[articleId]);
//retrieve all comments from identified user
...
//parses API call to pull the article id
...
//NEW CODE - Updates existing comment
...
//NEW CODE - Deletes existing comment
...
//NEW CODE - Records an comment upvote
...
//NEW CODE - Records an comment downvote
...
//helper function to do an upvote
...
//helper function to do a downvote
